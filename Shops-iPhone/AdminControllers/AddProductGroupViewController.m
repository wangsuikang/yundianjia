//
//  AddProductGroupViewController.m
//  Shops-iPhone
//
...
@interface AddProductGroupViewController () <UITextViewDelegate, UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation AddProductGroupViewController
...
//    _bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 370)];
    ...
    // 名字文本编辑框
    _nameTextFile = [[EditTextField alloc] initWithFrame:CGRectMake(CGRectGetMaxX(nameLabel.frame) + kSpace,kSpace, kScreenWidth - 200, 40)];
    _nameTextFile.placeholder = @" 输入名称";
    _nameTextFile.font = kMidSizeFont;
...
//    [_bgView addSubview:_typeComBox];
    ...
    // 中间分割view
    UIView *dismemberView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(nameLabel.frame) + 2 * kSpace, kScreenWidth, 1.5 * kSpace)];
    dismemberView.backgroundColor = kGrayColor;

    
    //    // 中间分割view
    //    UIView *dismemberView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_nameTextFile.frame) + 2 * kSpace, kScreenWidth, 1.5 * kSpace)];
    //    dismemberView.backgroundColor = kGrayColor;
    //
    //    [_bgView addSubview:dismemberView];
    
    // 描素
    UILabel *descLabel = [[UILabel alloc] initWithFrame:CGRectMake(4 * kSpace, CGRectGetMaxY(_nameTextFile.frame) + 2 * kSpace, 50, 20)];
    descLabel.text = @"描 述";
    ...
    // 添加textview
    _descTextView = [[UITextView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(descLabel.frame) + kSpace, CGRectGetMaxY(_nameTextFile.frame) + 1.5 * kSpace, kScreenWidth - CGRectGetMaxX(descLabel.frame) - 4 * kSpace, 150)];
...
//    }
    NSDictionary *params = @{@"user_session_key"        :   kNullToString(appDelegate.user.userSessionKey),
                             ...
                             @"shop_id"                 :   kNullToString(_shop_id),
                             @"name"                    :   kNullToString(_nameTextFile.text),
                             @"description"             :   kNullToString(_descTextView.text)};
    
    NSString *addNewGroupURL = [Tool buildRequestURLHost:kRequestHostWithPublic APIVersion:kAPIVersion1WithShops requestURL:kAdminAddNewGroup params:params];
    
    YunLog(@"AddNewGroupURL = %@", addNewGroupURL);
    
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    
    [manager POST:addNewGroupURL
      parameters:nil
         success:^(AFHTTPRequestOperation *operation, id responseObject) {
             YunLog(@"AddNewGroup responseObject = %@", responseObject);
             if ([kNullToString([[responseObject objectForKey:@"status"] objectForKey:@"code"]) isEqualToString:kSuccessCode])
             {
                 ...
                 [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationAddNewGroupSuccess object:nil];
             ...
             YunLog(@"AddNewGroupURL - error = %@", error);
         }];
}
