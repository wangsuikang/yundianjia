//
//  SettingFreeDetailViewController.m
//  Shops-iPhone
//
...
@interface SettingFreeDetailViewController ()

/// 三方库MBProgressHUD对象
@property (nonatomic, strong) MBProgressHUD *hud;
@end

@implementation SettingFreeDetailViewController
...
- (void)createUI
{
    ...
    UILabel *midLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(topLabel.frame), kScreenWidth, 30)];
    
    NSString *minValue = [[[_rulesArr firstObject] objectForKey:@"rule"] objectForKey:@"min_value"];
    
    midLabel.text = [minValue isEqualToString:@"0"] ? @"全场包邮" : [NSString stringWithFormat:@"全场满%@元包邮", minValue];
    ...
    UILabel *tittleLable = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(midLabel.frame) + 5, kScreenWidth, 25)];
    ...
    UILabel *bottomLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(tittleLable.frame), kScreenWidth, 20)];
         ...
         parameters:nil
            success:^(AFHTTPRequestOperation *operation, id responseObject) {
                ...
                if ([kNullToString([[responseObject objectForKey:@"status"] objectForKey:@"code"]) isEqualToString:kSuccessCode])
                {
                    ...
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"deleteActivitySucceedFreight" object:nil];
