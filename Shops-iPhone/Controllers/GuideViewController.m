...
@interface GuideViewController () <MYIntroductionDelegate, UIScrollViewDelegate/*, UIScrollViewDelegate, UIGestureRecognizerDelegate*/>
...
- (void)createPageView
{
    ...
    if (kIsiPhone) {
        if (kScreenHeight > 480) {
            imageNameArray = @[@"guide_one_iphone6", @"guide_two_iphone6", @"guide_three_iphone6"];
        } else {
            imageNameArray = @[@"guide_one_iphone4", @"guide_two_iphone4", @"guide_three_iphone4"];
        }
    } else {
        imageNameArray = @[@"guide_one_ipad", @"guide_two_ipad", @"guide_three_ipad"];
    }
    ...
    // 进入首页
    _enterBtn = [[UIButton alloc] initWithFrame:CGRectMake(kScreenWidth - 110, kScreenHeight - 60, 100, 50)];
    
    _enterBtn.backgroundColor = [UIColor clearColor];
    [_enterBtn setBackgroundImage:[UIImage imageNamed:@"welcomeButton"] forState:UIControlStateNormal];
...
- (void)enterBtnClick
{
    ...
    dispatch_once(&enterHome, ^{
        ...
        NSInteger user_type = [[[NSUserDefaults standardUserDefaults] objectForKey:@"userType"] integerValue];
        
        if (appDelegate.isLogin)
        {
            if (user_type == 1) { // 进入买家首页
                AppDelegate *appDelegate = kAppDelegate;
...
{
    YunLog(@"sender.currentPage = %ld", (long)sender.currentPage);
