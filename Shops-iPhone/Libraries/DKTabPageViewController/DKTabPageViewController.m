...
- (instancetype)initWithSourceController:(ShopInfoViewController *)sourceController Frame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if (self) {
        self.sourceController = sourceController;
        UIView *wrapper = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 64)];
        ...
        header.delegate = self.sourceController;
        [header highlightButtonAtIndex:self.sourceController.currentIndex];
...
//        [fromLowToHigh addTarget:header.delegate action:@selector(orderProductFromLowToHigh:) forControlEvents:UIControlEventTouchUpInside];
...
//        [fromHighToLow addTarget:header.delegate action:@selector(orderProductFromHighToLow:) forControlEvents:UIControlEventTouchUpInside];
...
//        switch (self.sourceController.subIndexOfPriceOrder) {
//            case 1:
...
@property (nonatomic, weak) ShopInfoViewController *sourceController;
@end

@implementation DKTabPageViewController
- (instancetype)initWithItems:(NSArray *)items sourceController:(ShopInfoViewController *)sourceController
{
...
- (DKTabPageBar *)tabPageBar {
    if (_tabPageBar == nil) {
        _tabPageBar = [[DKTabPageBar alloc] initWithSourceController:self.sourceController Frame:CGRectMake(0, 0, kScreenWidth, self.sourceController.headerHeight)];
    }
    return _tabPageBar;
}
...
- (UIScrollView *)mainScrollView {
    if (_mainScrollView == nil) {
        if (DKTABPAGE_IOS_VERSION_GREATER_THAN_7) {
            _mainScrollView = [[CustomScrollView alloc] init];
        } else {
...
- (void)setSelectedIndexByIndex:(NSInteger)newIndex{
    ...
    if ([selectedItem isKindOfClass:[DKTabPageViewControllerItem class]]) {
        ...
        if (newIndex == 3) {
           // self.tabPageBar.frame = CGRectMake(0, 0, kScreenWidth, 64 + priceRowHeight);
        }
       
        if ([self.sourceController respondsToSelector:@selector(swipeToControllerIndex:)]) {
            [self.sourceController swipeToControllerIndex:newIndex];
            [self.tabPageBar.header highlightButtonAtIndex:newIndex];
        }
