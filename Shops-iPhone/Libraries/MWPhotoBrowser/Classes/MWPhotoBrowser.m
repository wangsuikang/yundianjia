...
- (void)viewDidLoad {
    ...
    // 图片底部描述和按钮
    _bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, kScreenHeight - 160, kScreenWidth, 160)];
    
    if (kDeviceOSVersion < 7.0) {
        _bottomView.frame = CGRectMake(0, kScreenHeight - 228, kScreenWidth, 160);
    }
    
    [self.view addSubview:_bottomView];
    
    UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(15, 0, kScreenWidth - 30, 20)];
    title.tag = 101;
    title.font = kNormalFont;
    title.textColor = [UIColor whiteColor];
    title.backgroundColor = kClearColor;
    
    NSDictionary *content = [_delegate photoBrowser:self titleAndDescriptionAtIndex:_currentPageIndex];
    ...
    UIScrollView *textScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 25, kScreenWidth, 100)];
    textScrollView.showsVerticalScrollIndicator = NO;
    textScrollView.contentSize = CGSizeZero;
    textScrollView.tag = 102;
    textScrollView.backgroundColor = kClearColor;
    
    if (![content[@"text"] isEqualToString:@""]) {
        NSString *text = [content[@"text"] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
        
        LineSpaceLabel *label = [[LineSpaceLabel alloc] init];
        label.numberOfLines = 0;
        label.text = text;
        label.textColor = [UIColor whiteColor];
        label.font = kSmallFont;
        
        CGFloat height = [label labelHight:kScreenWidth - 30];
        
        label.frame = CGRectMake(15, 0, kScreenWidth - 30, height);
        
        [textScrollView addSubview:label];
        
        textScrollView.contentSize = CGSizeMake(kScreenWidth, height);
    } else {
        textScrollView.contentSize = CGSizeZero;
    }
    
    [_bottomView addSubview:textScrollView];
    
//    UIButton *addToCart = [[UIButton alloc] initWithFrame:CGRectMake(kScreenWidth - 200, _bottomView.frame.size.height - 40, 100, 32)];
...
//    
    UIButton *buy = [[UIButton alloc] initWithFrame:CGRectMake(kScreenWidth - 90, _bottomView.frame.size.height - 40, 80, 32)];
    ...
    [buy setTitle:@"下载图片" forState:UIControlStateNormal];
    [buy setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [buy addTarget:self action:@selector(downloadImage) forControlEvents:UIControlEventTouchUpInside];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if ([_delegate respondsToSelector:@selector(photoBrowser:titleAndDescriptionAtIndex:)]) {
        NSDictionary *content = [_delegate photoBrowser:self titleAndDescriptionAtIndex:_currentPageIndex];
        
        UILabel *label = (UILabel *)[_bottomView viewWithTag:101];
        ...
        UIScrollView *scrollView = (UIScrollView *)[_bottomView viewWithTag:102];
...
//        hidden = NO;
    if (![self numberOfPhotos] || _gridController)
        hidden = NO;
    ...
    if (hidden) {
        [UIView animateWithDuration:0.5
                         animations:^{
                             _bottomView.frame = CGRectMake(0, kScreenHeight, _bottomView.frame.size.width, _bottomView.frame.size.height);
                         }];
    } else {
        [UIView animateWithDuration:0.5
                         animations:^{
                             _bottomView.frame = CGRectMake(0, kScreenHeight - _bottomView.frame.size.height, _bottomView.frame.size.width, _bottomView.frame.size.height);
                         }];
    }
}
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    ...
    if (error != NULL) {
        if (error.code == -3310) {
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"图片存储失败"
                                                                message:@"请在 设置-隐私-照片 中允许云店家访问您的相册"
                                                               delegate:nil
            ...
            [self showProgressHUDCompleteMessage:@"图片存储失败"];
        }
    } else {
        [self showProgressHUDCompleteMessage:@"图片已保存到相册"];
    }
...
//    [self showProgressHUDCompleteMessage:error ? @"图片存储失败" : @"图片已保存到相册"];
//    [self hideControlsAfterDelay]; // Continue as normal...
}
...
#pragma mark - Rujax Actions -

//- (void)goToCart
//{
//    if ([_delegate respondsToSelector:@selector(photoBrowser:bottomButtonPressAtIndex:)]) {
//        [_delegate photoBrowser:self bottomButtonPressAtIndex:0];
//    }
//}
//
//- (void)addToCart
//{
//    if ([_delegate respondsToSelector:@selector(photoBrowser:bottomButtonPressAtIndex:)]) {
//        [_delegate photoBrowser:self bottomButtonPressAtIndex:1];
//    }
//}
//
- (void)downloadImage
{
    for (MWZoomingScrollView *page in _visiblePages) {
        if (page.index == _currentPageIndex) {
            [self showProgressHUDWithMessage:@"努力保存中..."];
            
            UIImageWriteToSavedPhotosAlbum(page.photoImageView.image, self, @selector(image:didFinishSavingWithError:contextInfo:), nil);
