...
-(void)initializeViewComponents{
    ...
    YunLog(@"back_frame = %@", NSStringFromCGRect(self.BackgroundImageView.frame));
    ...
    YunLog(@"back_frame = %@", NSStringFromCGRect(self.MasterScrollView.frame));
    
    //Page Control
    self.PageControl = [[UIPageControl alloc] initWithFrame:CGRectMake((self.frame.size.width - kPageControlWidth)/2, self.frame.size.height - 40, kPageControlWidth, 37)];
    self.PageControl.currentPage = 0;
    self.PageControl.enabled = NO;
    self.PageControl.pageIndicatorTintColor = [UIColor whiteColor];
    self.PageControl.currentPageIndicatorTintColor = [UIColor lightGrayColor];
    YunLog(@"back_frame = %@", NSStringFromCGRect(self.PageControl.frame));
    
    [self addSubview:self.PageControl];
    
    //Get skipString dimensions
    NSString *skipString = @"进入首页"; // NSLocalizedString(@"Skip", nil);
    CGFloat skipStringWidth = 0;
    kSkipButtonFont = [UIFont fontWithName:@"AppleGothic" size:16];
...
//        skipStringWidth = [skipString sizeWithFont:kSkipButtonFont constrainedToSize:CGSizeMake(MAXFLOAT, MAXFLOAT) lineBreakMode:NSLineBreakByWordWrapping].width;
        skipStringWidth = [skipString sizeWithFont:kSkipButtonFont size:CGSizeMake(MAXFLOAT, MAXFLOAT)].width;
    }
    
    //Left Skip Button
    self.LeftSkipButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.LeftSkipButton.frame = CGRectMake(10, self.frame.size.height - 40, 46, 37);
    ...
    self.LeftSkipButton.titleLabel.textAlignment = NSTextAlignmentCenter;
    [self.LeftSkipButton setTitleColor:[UIColor orangeColor] forState:UIControlStateNormal];
    ...
    self.RightSkipButton.frame = CGRectMake(self.frame.size.width - skipStringWidth - kLeftRightSkipPadding, self.frame.size.height - 40, skipStringWidth, 37);
    ...
    self.RightSkipButton.titleLabel.textAlignment = NSTextAlignmentCenter;
    [self.RightSkipButton setTitleColor:[UIColor orangeColor] forState:UIControlStateNormal];
...
//    self.BackgroundColorView.backgroundColor = self.UserBackgroundColor;
    ...
    YunLog(@"BackgroundColorView = %@", NSStringFromCGRect(self.BackgroundColorView.frame));
    
    YunLog(@"back_frame = %@", NSStringFromCGRect(self.frame));
