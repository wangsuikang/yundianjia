...
//    kTitleFont = [UIFont boldSystemFontOfSize:28];
    kTitleFont = [UIFont fontWithName:@"Helvetica Neue" size:28];
    kTitleTextColor = [UIColor whiteColor];
//    kDescriptionFont = [UIFont systemFontOfSize:18];
    kDescriptionFont = [UIFont fontWithName:@"CourierNewPSMT" size:18];
...
-(void)buildPanelWithFrame:(CGRect)frame{
    ...
    if (self.PanelHeaderView) {
        ...
        YunLog(@"back_frame = %@", NSStringFromCGRect(self.PanelHeaderView.frame));
...
//        panelTitleHeight = [self.PanelTitle sizeWithFont:kTitleFont constrainedToSize:CGSizeMake(frame.size.width - 2 * kLeftRightMargins, MAXFLOAT) lineBreakMode:NSLineBreakByWordWrapping].height;
        panelTitleHeight = [self.PanelTitle sizeWithFont:kTitleFont size:CGSizeMake(frame.size.width - 2 * kLeftRightMargins, MAXFLOAT)].height;
...
//        panelDescriptionHeight = [self.PanelDescription sizeWithFont:kDescriptionFont constrainedToSize:CGSizeMake(frame.size.width - 2 * kLeftRightMargins, MAXFLOAT) lineBreakMode:NSLineBreakByWordWrapping].height;
        panelDescriptionHeight = [self.PanelDescription sizeWithFont:kDescriptionFont size:CGSizeMake(frame.size.width - 2 * kLeftRightMargins, MAXFLOAT)].height;
    }
    ...
    YunLog(@"back_frame = %@", NSStringFromCGRect(self.PanelTitleLabel.frame));
...
//     [self addSubview:self.PanelSeparatorLine];
    ...
    YunLog(@"back_frame = %@", NSStringFromCGRect(self.PanelDescriptionLabel.frame));
...
//        self.PanelImageView.frame = CGRectMake(kLeftRightMargins, runningYOffset, self.frame.size.width - 2 * kLeftRightMargins, self.frame.size.height - runningYOffset - kBottomPadding);
        self.PanelImageView.frame = kScreenBounds;
        YunLog(@"self.imageview = %@", NSStringFromCGRect(self.PanelImageView.frame));
...
- (void) showContent
{
    ...
    YunLog(@"initialImageFrame = %@", NSStringFromCGRect(initialImageFrame));
