...
- (id)initWithFrame:(CGRect)frame total:(NSNumber *)total current:(NSNumber *)current clockwise:(BOOL)clockwise {
    ...
    return [self initWithFrame:frame total:total current:current clockwise:clockwise shadow:NO shadowColor:[UIColor clearColor] displayCountingLabel:YES overrideLineWidth:@8.0f addMidLabel:YES midLabelTitle:@"销售订单"];
...
displayCountingLabel:(BOOL)displayCountingLabel
  overrideLineWidth:(NSNumber *)overrideLineWidth
        addMidLabel:(BOOL)isAddLabel
      midLabelTitle:(NSString *)title
{
...
//        [_countingLabel setCenter:CGPointMake(self.frame.size.width/2.0f, self.frame.size.height/2.0f + 20)];
...
//        }
//        
//        if (_displayCountingLabel && _isAddLabel) {
//            _countingLabel.text = [NSString stringWithFormat:@"%d", [total intValue]];
//        }
        
        UILabel *countLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 100.0, 50)];
        [countLabel setTextAlignment:NSTextAlignmentCenter];
        [countLabel setFont:kLangeFont];
        [countLabel setTextColor:[UIColor whiteColor]];
        [countLabel setBackgroundColor:[UIColor clearColor]];
        [countLabel setCenter:CGPointMake(self.frame.size.width/2.0f, self.frame.size.height/2.0f + 20)];
        countLabel.text = [NSString stringWithFormat:@"%d", [total intValue]];
        
        [self addSubview:countLabel];
        
        UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 30.0)];
        [titleLabel setTextAlignment:NSTextAlignmentCenter];
        [titleLabel setFont:kBigFont];
        [titleLabel setTextColor:[UIColor blackColor]];
        [titleLabel setBackgroundColor:[UIColor clearColor]];
        [titleLabel setCenter:CGPointMake(self.frame.size.width/2.0f, self.frame.size.height/2.0f - 20)];
