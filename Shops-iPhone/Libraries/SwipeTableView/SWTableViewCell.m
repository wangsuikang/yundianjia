...
- (void)layoutSubviews
{
    ...
    YunLog(@"self.cellScrollView.frame = %@", NSStringFromCGRect(self.cellScrollView.frame));
    ...
    if (!self.cellScrollView.isTracking && !self.cellScrollView.isDecelerating)
    {
        ...
        YunLog(@"frame = %@", NSStringFromCGPoint(self.cellScrollView.contentOffset));
...
- (void)setFrame:(CGRect)frame
{
    ...
    YunLog(@"frame = %@", NSStringFromCGRect(frame));
...
- (void)rightUtilityButtonHandler:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(swipeableTableViewCell:didTriggerRightUtilityButtonWithIndex: indexPath:)])
    {
        [self.delegate swipeableTableViewCell:self didTriggerRightUtilityButtonWithIndex:utilityButtonIndex indexPath:indexPath];
    }
}
...
- (void)updateCellState
{
    if(layoutUpdating == NO)
    {
        ...
        YunLog(@"frame_edit = %@", NSStringFromCGRect(frame));
        ...
        YunLog(@"left.constant = %f", self.leftUtilityClipConstraint.constant);
        YunLog(@"left.constant = %f", self.rightUtilityClipConstraint.constant);
